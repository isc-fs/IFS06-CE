...
bool CPU_MOD::parse(uint32_t id, uint8_t* buf, uint32_t t) {
    if (id == 0x100) {
        ...
        DC_BUS = (buf[0] << 8) + buf[1]; // This direction sends the voltage in DC_BUS
        if (DC_BUS > 280) { //(DC_BUS>0.9*voltage_acum)
            error = CPU_BUS_LINE_OK;
            if (module_send_message_NoExtId_CAN1(CANID_send, currentState, 1) != HAL_OK)
